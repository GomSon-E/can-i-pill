...
const DELETING_MEDS = [
  { icon: '💊', name: '혈압약', detail: '암로디핀 5mg' },
  { icon: '💊', name: '혈압약', detail: '로사르탄 50mg' },
  { icon: '🟡', name: '위장보호약', detail: '오메프라졸 20mg' },
]
  ...
  return (
    <div data-testid="page-sub-03" style={{ width: 360, height: 780, backgroundColor: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: "'Pretendard Variable', 'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ height: 52, display: 'flex', alignItems: 'center', paddingLeft: 8, paddingRight: 22, flexShrink: 0, borderBottom: '1px solid #F0F0F3' }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: '#1A1B22', marginLeft: 4 }}>처방전 교체</span>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', paddingLeft: 22, paddingRight: 22, paddingTop: 0 }}>
        {/* 교체 모드 배너 */}
        <div style={{ backgroundColor: 'rgb(253,236,236)', borderRadius: 14, padding: '15px 16px', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#C62828' }}>교체 모드</span>
          </div>
          <div style={{ fontSize: 17, color: '#1A1B22', marginBottom: 6, lineHeight: '24px' }}>
            처방전을 교체합니다
          </div>
        </div>

        {/* 삭제될 약 목록 */}
        <div style={{ padding: '12px 16px', marginBottom: 21, backgroundColor: '#F8F8FA' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#54555C', marginBottom: 8 }}>
            이 약들이 삭제되고 새 처방전으로 교체됩니다
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {DELETING_MEDS.map((med, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 13 }}>{med.icon}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#1A1B22' }}>{med.name}</span>
                <span style={{ fontSize: 13, color: '#8B8C96' }}>({med.detail})</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ border: '1.5px solid #E4E5E9', borderRadius: 16, padding: 16, marginBottom: 12, position: 'relative', overflow: 'hidden', minHeight: 170, boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            <div style={{ fontSize: 13, color: '#8B8C96' }}>· 교체될 새 약</div>
