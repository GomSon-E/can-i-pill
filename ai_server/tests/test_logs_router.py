...
def test_get_logs_requests_respects_offset(monkeypatch, tmp_path):
    metrics._METRICS_BUFFER.clear()
    monkeypatch.setenv("AI_METRICS_CSV_PATH", str(tmp_path / "metrics.csv"))
    metrics.record_metric("/ocr", 200, 100.0, True)
    ...
    response = client.get("/logs/requests", params={"limit": 1, "offset": 1})
    ...
    assert data["requests"][0]["endpoint"] == "/label"
...
def test_get_logs_analyze_respects_offset(monkeypatch, tmp_path):
    analyze_logs._LOG_BUFFER.clear()
    monkeypatch.setenv("AI_ANALYZE_LOGS_PATH", str(tmp_path / "analyze_logs.jsonl"))
    result = {"type": "analysis", "level": "safe", "trace": []}
    analyze_logs.record_analyze_log("첫번째", "", result, 1.0)
    analyze_logs.record_analyze_log("두번째", "", result, 2.0)
    analyze_logs.record_analyze_log("세번째", "", result, 3.0)

    response = client.get("/logs/analyze", params={"limit": 1, "offset": 1})
    ...
    assert data["limit"] == 1
    assert data["offset"] == 1
    assert data["logs"][0]["question"] == "두번째"
